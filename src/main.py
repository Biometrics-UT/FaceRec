...
def draw_faces_scaled(face_locations, face_names, face_landmarks_list, frame, scale=4):
    ...
    for (top, right, bottom, left), name, face_landmarks in zip(face_locations, face_names, face_landmarks_list):
        ...
        cv2.polylines(frame, np.int32([face_landmarks['right_eyebrow']])*scale, isClosed=True, color=(68, 54, 39, 128))
        cv2.polylines(frame, np.int32([face_landmarks['left_eyebrow']])*scale, isClosed=True, color=(68, 54, 39, 150), thickness=5)
        cv2.polylines(frame, np.int32([face_landmarks['right_eyebrow']])*scale, isClosed=True, color=(68, 54, 39, 150), thickness=5)
        ...
        cv2.polylines(frame, np.int32([face_landmarks['top_lip']])*scale, isClosed=True, color=(150, 0, 0, 64), thickness=5)
        cv2.polylines(frame, np.int32([face_landmarks['bottom_lip']])*scale, isClosed=True, color=(150, 0, 0, 64), thickness=5)
